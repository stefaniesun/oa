...
public class LoginSvcImp implements LoginSvc{
	...
	@Override
	public Map<String, Object> loginByImeiOper(String imei,int indateHours) {
		if(imei==null || "".equals(imei)){
			return ReturnUtil.returnMap(0, "设备号不存在");
		}

		@SuppressWarnings("unchecked")
		List<String> list = commonDao.getSqlQuery("SELECT username FROM security_user t WHERE SUBSTR(MD5(t.imei) FROM 9 FOR 16)='"+imei+"'").list();
		if(list==null || list.size()!=1){
			return ReturnUtil.returnMap(0, "设备号未绑定");
		}
		
		String hql = "from SecurityUser s where s.username = '"+list.get(0)+"'";
		SecurityUser securityUser = (SecurityUser)commonDao.queryUniqueByHql(hql);
		if(securityUser==null){
